...
const auraFragmentShader = `
  ...
  float sampleMorphingProfile(
    ...
    float profileIndex = floor(profileTime);
    float profileBlend = smoothstep(0.0, 1.0, fract(profileTime));
    float rowA = hash11(profileIndex + seed);
    float rowB = hash11(profileIndex + 1.0 + seed);
    float profileA = texture2D(noiseTexture, vec2(angle * tiling, rowA)).r;
    float profileB = texture2D(noiseTexture, vec2(angle * tiling, rowB)).r;
    return mix(profileA, profileB, profileBlend);
  }
  ...
  void main() {
    ...
    float angle = atan(centerUV.y, centerUV.x) / TAU + 0.5;

    // The source texture was authored with Photoshop Polar to Rectangular.
    // Radius maps to X and the fixed quarter turn restores its authored orientation.
    // Time never changes the angle, so recognizable spikes cannot orbit the aura.
    float polarAngle = fract(angle + 0.25);
    float waveTime = u_time * u_motionScale;
    float fullMotion = step(0.5, u_motionScale);
    float profileSpeedMultiplier = mix(1.0, 2.0, fullMotion);
    float macroProfile = sampleMorphingProfile(
      u_noiseA,
      polarAngle,
      waveTime * 5.0 * profileSpeedMultiplier,
      5.0,
      17.0
    );
    float supportingProfile = sampleMorphingProfile(
      u_noiseB,
      polarAngle,
      waveTime * 5.0 * profileSpeedMultiplier,
      7.0,
    ...
    float microProfile = sampleMorphingProfile(
      u_noiseB,
      polarAngle,
      waveTime * 12.0 * profileSpeedMultiplier,
      13.0,
      91.0
    );

    float motionStrength = mix(0.18, 1.0, clamp(u_motionScale, 0.0, 1.0));
    float combinedMacro = macroProfile * 0.68 + supportingProfile * 0.32;
    float macroSpike = pow(smoothstep(0.38, 0.92, combinedMacro), 2.2);
    float microSpike = pow(smoothstep(0.5, 0.94, microProfile), 2.0);
    float macroDistortion = (0.5 - combinedMacro) * 0.1 - macroSpike * 0.14;
    float microDistortion = (0.5 - microProfile) * 0.025 - microSpike * 0.045;
    float rawDistortion = macroDistortion + microDistortion;
    float amplifiedDistortion = rawDistortion * 10.0;
    float limitedDistortion = amplifiedDistortion /
      (1.0 + abs(amplifiedDistortion) / 0.5);
    float radialDistortion = mix(
      rawDistortion,
      limitedDistortion,
      fullMotion
    );
    float radialUv = clamp(
      radius * 2.5 + radialDistortion * motionStrength,
      0.0,
      0.999
    );
    float maskValue = texture2D(u_mask, vec2(radialUv, polarAngle)).r;

    // A single mask sample is authoritative for the entire layer: gray is the
    // translucent body, white is its HDR contour, and black is discarded.
    float maskCoverage = smoothstep(0.025, 0.18, maskValue);
    float brightContour = smoothstep(0.68, 0.94, maskValue);
    float alpha = mix(0.22, 0.82, brightContour) * maskCoverage;
    ...
    float brightness = mix(0.9, u_emission, brightContour);
...
export class AuraVfx3 {
  ...
  private readonly auraGeometry: THREE.PlaneGeometry;
  private readonly auraMesh: THREE.Mesh<THREE.PlaneGeometry, THREE.ShaderMaterial>;
  ...
  constructor(options: AuraVfx3Options = {}) {
    this.group.name = "AuraVFX_3";
    ...
    this.auraGeometry = new THREE.PlaneGeometry(5, 5);
    this.auraMesh = new THREE.Mesh(this.auraGeometry, this.auraMaterial);
    this.auraMesh.name = "AuraVFX_3 Vertical Billboard";
    this.auraMesh.position.set(0, 0.3, 0);
    ...
    this.ringMesh.name = "AuraVFX_3 Ground Rings";
    ...
    this.sparkMesh.name = "AuraVFX_3 Sparks";
  ...
  private updateBillboard(camera: THREE.Camera): void {
    camera.getWorldPosition(this.cameraWorldPosition);
    this.group.updateWorldMatrix(true, false);
    this.cameraLocalPosition.copy(this.cameraWorldPosition);
    this.group.worldToLocal(this.cameraLocalPosition);
    const offsetX = this.cameraLocalPosition.x - this.auraMesh.position.x;
    const offsetZ = this.cameraLocalPosition.z - this.auraMesh.position.z;
    this.auraMesh.rotation.y = Math.atan2(offsetX, offsetZ);
  }
}
